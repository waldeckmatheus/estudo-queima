...
|---|---|---|---|---|
| 1 | **Nível de evidência não classificado formalmente** | Nenhuma recomendação é classificada por nível de evidência (Oxford CEBM, GRADE). O leitor não distingue: "baseado em meta-análise de RCTs" de "baseado em mecanismo fisiológico sem RCT em humanos". Para 30% das afirmações mais específicas, a evidência é nível 3–5 (observacional, opinião de expert, modelos animais). | Leitor assume que tudo tem mesma robustez | Incluir ao lado de cada recomendação: [NE:1a], [NE:2b], [NE:5] ou equivalente GRADE (⊕⊕⊕⊕ a ⊕○○○) |
...
O protocolo é **metodologicamente superior à maioria dos protocolos de dieta** disponíveis: 125+ referências rastreáveis, meta-análises como base das recomendações centrais, transparência sobre incertezas e mecanismos fisiológicos explicitados. As **lacunas** são: ausência de classificação formal de nível de evidência (o leitor não sabe o que é forte vs. fraco), viés de seleção de referências (sem busca sistemática), extrapolação de atletas para população geral, tamanhos amostrais pequenos não reportados e conflito de interesse não declarado. Um metodologista real **reconheceria a qualidade acima do padrão** e recomendaria: (1) classificação GRADE de cada recomendação, (2) declaração de CoI, (3) especificação de populações estudadas e (4) apresentação de evidências contrastantes.
