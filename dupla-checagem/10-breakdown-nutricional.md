# Dupla Checagem — Cap. 10: Breakdown Nutricional

**Arquivo:** `10-breakdown-nutricional.md`
**Data:** 2025-04-05

## Estrutura

| Métrica | Valor |
|---------|-------|
| Seções (###) | 2 (10.1 Sem Azeite, 10.2 Com Azeite) |
| Refs únicas | 2 ([web:52, 68]) |
| Callouts | 0 |
| Refs cruzadas | Seção 6.2 |

## Verificação de Conteúdo

- ✅ Tabelas de distribuição calórica claras (978–1.284 kcal)
- ✅ Referências a limiares de segurança VLCD corretas

## Alertas

| # | Severidade | Descrição |
|---|-----------|-----------|
| 1 | Média | Capítulo breve (~20 linhas); falta detalhamento sobre como adaptar porções para peso individual |
| 2 | Média | Ap. A3 referencia "ver Cap. 10 para recalcular" mas Cap. 10 não contém exemplos de recálculo |

## Resultado: ⚠️ APROVADO COM RESSALVAS (2 alertas médios)
